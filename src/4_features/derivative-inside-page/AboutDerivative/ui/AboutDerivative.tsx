...
export const AboutDerivative: React.FC = () => {
    ...
    const { data: derivativeInfo, isLoading } = useGetDerivativeExchangeQuery(
        String(idDerivative)
    )
    return (
            ...
            {derivativeInfo && (
                <div className={'about-crypto_container'}>
                    <div className={'derivative-info'}>
                        <span className={'rank'}>
                            Futures #{derivativeInfo.number_of_futures_pairs}
                        </span>
                        <div className={'derivative'}>
                            <img
                                src={derivativeInfo.image}
                                alt={derivativeInfo.name}
                            />
                            <h1>{derivativeInfo.name}</h1>
                        </div>
                        {derivativeInfo.description && (
                            <p>{derivativeInfo.description}</p>
                        )}
                    </div>
                    <div className={'cards-container'}>
                        ...
                        <div className={'card'}>
                            <span>Website</span>
                            <Link href={derivativeInfo.url}>
                                {derivativeInfo.url.replace('https://', '')}
                            </Link>
                        </div>
                    </div>
                </div>
