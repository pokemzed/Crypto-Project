...
export const AboutExchange: React.FC = () => {
    ...
    const {data: exchange, isLoading} = useGetExchangeQuery(String(exchangeName))
    return(
            ...
            {exchange && (
                <div className={"about-exchange-container"}>
                    <div className={"exchange-info"}>
                        <span className={"rank"}>Rank #{exchange.trust_score_rank}</span>
                        <div className={"exchange"}>
                            <img src={exchange.image} alt={`${exchange.name}-image`}/>
                            <h1>{exchange.name}</h1>
                        </div>
                    </div>
                    <div className={"cards-info"}>
                        <div className={"card"}>
                            ...
                            <h4>{exchange.trade_volume_24h_btc.toLocaleString('en')} (BTC)</h4>
                        </div>
                        <div className={"card"}>
